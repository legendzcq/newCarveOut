...
@implementation FFTVerAlertView
...
- (instancetype)initWithVer:(NSString *)Ver message:(NSString *)message sureBtn:(NSString *)sureTitle cancleBtn:(NSString *)cancleTitle
{
    if (self == [super init]) {
        ...
        UIImageView * imagev = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, AlertW, 95)];
        imagev.image = kImageNamed(@"fft_home_verImg");
        [self.alertView addSubview:imagev];
        
        
        self.versionLbl = [[UILabel alloc] initWithFrame:CGRectMake(XLSpace, CGRectGetMaxY(imagev.frame)+16, AlertW-XLSpace, 12)];
        self.versionLbl.numberOfLines = 0;
        self.versionLbl.textColor = [UIColor colorWithHexString:@"a4a4a4"];
        self.versionLbl.text = [NSString stringWithFormat:@"最新版本:%@",Ver];
        self.versionLbl.font = [UIFont systemFontOfSize:11];
        [self.alertView addSubview:self.versionLbl];
        
        UILabel* alertLbl = [[UILabel alloc] initWithFrame:CGRectMake(XLSpace, CGRectGetMaxY(self.versionLbl.frame)+12, AlertW-XLSpace, 12)];
        alertLbl.numberOfLines = 0;
        alertLbl.textColor = [UIColor colorWithHexString:@"333333"];
        alertLbl.text = @"更新说明：";
        alertLbl.font = [UIFont systemFontOfSize:11];
        [self.alertView addSubview:alertLbl];
        
        
        if (message) {
            
            self.msgLbl = [self GetAdaptiveLable:CGRectMake(XLSpace, CGRectGetMaxY(alertLbl.frame)+XLSpace, AlertW-2*XLSpace, 20) AndText:message andIsTitle:NO];
            ...
            self.msgLbl.frame = self.versionLbl?CGRectMake((AlertW-msgW)/2, CGRectGetMaxY(alertLbl.frame)+XLSpace, msgW, msgH):CGRectMake((AlertW-msgW)/2, 2*XLSpace, msgW, msgH);
        }
        
        self.lineView = [[UIView alloc] init];
        self.lineView.frame = self.msgLbl?CGRectMake(0, CGRectGetMaxY(self.msgLbl.frame)+2*XLSpace, AlertW, 1):CGRectMake(0, CGRectGetMaxY(self.versionLbl.frame)+2*XLSpace, AlertW, 1);
        ...
        if(sureTitle && cancleTitle){
            ...
            self.sureBtn.titleLabel.font = [UIFont boldSystemFontOfSize:14.f];
...
-(UILabel *)GetAdaptiveLable:(CGRect)rect AndText:(NSString *)contentStr andIsTitle:(BOOL)isTitle
{
    ...
    if (isTitle) {
        ...
        contentLbl.font = [UIFont systemFontOfSize:11.0];
    }
