...
@interface JLMImagePicker()<UINavigationControllerDelegate, UIImagePickerControllerDelegate,VPImageCropperDelegate>{
    BOOL isScale;
    double _scale;
}
...
@end
@implementation JLMImagePicker
#pragma  mark -- 单例 --
+ (instancetype)sharedInstance{
    static dispatch_once_t ETToken;
    static JLMImagePicker *sharedInstance = nil;
    dispatch_once(&ETToken, ^{
        sharedInstance = [[JLMImagePicker alloc] init];
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info{
    ...
    if (isScale) {
        self.imageCropperController = [[VPImageCropperViewController alloc] initWithImage:image cropFrame:CGRectMake(0, (kHeight-kWidth*_scale)/2, kWidth, kWidth*_scale) limitScaleRatio:5];
        self.imageCropperController.delegate = self;
        [picker pushViewController:self.imageCropperController animated:YES];
    }else{
        [picker dismissViewControllerAnimated:YES completion:^{}];
        if (self.imagePickerDidFinished) {
            self.imagePickerDidFinished(image);
        }
...
- (void)imageCropper:(VPImageCropperViewController *)cropperViewController didFinished:(UIImage *)editedImage{
    ...
    if (self.imagePickerDidFinished) {
        self.imagePickerDidFinished(editedImage);
    }
}
#pragma mark - Getters
