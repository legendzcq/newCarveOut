...
+ (instancetype)buttonWithTYSe:(UIButtonType)buttonType {
    YSTabItem *item = [super buttonWithType:buttonType];
    [item setup];
    return item;
}
...
- (void)setup {
    ...
    _badgeStyle = YSTabItemBadgeStyleNumber;
    self.badge = 0;
}

/**
 *  覆盖父类的setHighlighted:方法，按下YSTabItem时，不高亮该item
 */
...
- (void)setBadgeStyle:(YSTabItemBadgeStyle)badgeStyle {
    _badgeStyle = badgeStyle;
    [self updateBadge];
}

- (void)updateBadge {
    if (self.badgeStyle == YSTabItemBadgeStyleNumber) {
        if (self.badge == 0) {
    ...
    } else if (self.badgeStyle == YSTabItemBadgeStyleDot) {
