...
@property (nonatomic, strong) YSTabItem *specialItem;
@property (nonatomic, copy) void (^specialItemHandler)(YSTabItem *item);

// 选中背景
@property (nonatomic, strong) UIImageView *itemSelectedBgImageView;

// 选中背景相对于YSTabItem的insets
...
- (void)awakeFromNib {
    ...
    line.backgroundColor = kColor(226, 227, 228);
}
...
- (void)setItems:(NSArray *)items {
    ...
    // 初始化每一个item
    for (YSTabItem *item in self.items) {
...
- (void)setTitles:(NSArray *)titles {
    ...
    for (NSString *title in titles) {
        YSTabItem *item = [[YSTabItem alloc] init];
        item.title = title;
...
- (void)updateItemsFrame {
    ...
    if (self.scrollView) {
        ...
        for (NSUInteger index = 0; index < self.items.count; index++) {
            YSTabItem *item = self.items[index];
            CGFloat width = 0;
            // item的宽度为一个固定值
            if (self.itemWidth > 0) {
...
- (void)setSelectedItemIndex:(NSUInteger)selectedItemIndex {
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(ys_tabBar:shouldSelectItemAtIndex:)]) {
        BOOL should = [self.delegate ys_tabBar:self shouldSelectItemAtIndex:selectedItemIndex];
        if (!should) {
            return;
        }
    }
    if (self.delegate && [self.delegate respondsToSelector:@selector(ys_tabBar:willSelectItemAtIndex:)]) {
        [self.delegate ys_tabBar:self willSelectItemAtIndex:selectedItemIndex];
    }
    
    if (_selectedItemIndex != NSNotFound) {
        YSTabItem *oldSelectedItem = self.items[_selectedItemIndex];
    ...
    YSTabItem *newSelectedItem = self.items[selectedItemIndex];
    ...
    if (self.delegate && [self.delegate respondsToSelector:@selector(ys_tabBar:didSelectedItemAtIndex:)]) {
        [self.delegate ys_tabBar:self didSelectedItemAtIndex:selectedItemIndex];
    }
}
...
- (void)tabItemClicked:(YSTabItem *)item {
    self.selectedItemIndex = item.index;
}

- (void)specialItemClicked:(YSTabItem *)item {
    if (self.specialItemHandler) {
        self.specialItemHandler(item);
    }
}

- (YSTabItem *)selectedItem {
...
- (void)setSpecialItem:(YSTabItem *)item afterItemWithIndex:(NSUInteger)index tapHandler:(void (^)(YSTabItem *item))handler {
...
- (void)updateSeperators {
    if (self.itemSeparatorColor) {
        ...
        [self.items enumerateObjectsUsingBlock:^(YSTabItem * _Nonnull item, NSUInteger idx, BOOL * _Nonnull stop) {
            if (idx > 0) {
...
- (void)updateSubViewsWhenParentScrollViewScroll:(UIScrollView *)scrollView {
    ...
    YSTabItem *leftItem = self.items[leftIndex];
    YSTabItem *rightItem = nil;
