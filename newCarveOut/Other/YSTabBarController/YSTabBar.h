...
/**
 *  TabItems，提供给YSTabBarController使用，一般不手动设置此属性
 */
@property (nonatomic, copy) NSArray <YSTabItem *> *items;
...
/**
 *  添加一个特殊的YSTabItem到tabBar上，此TabItem不包含在tabBar的items数组里
 *  主要用于有的项目需要在tabBar的中间放置一个单独的按钮，类似于新浪微博等。
 *  此方法仅适用于不可滚动类型的tabBar
 *
 *  @param item    YSTabItem对象
 *  @param index   将其放在此index的item后面
 *  @param handler 点击事件回调
 */
- (void)setSpecialItem:(YSTabItem *)item
    afterItemWithIndex:(NSUInteger)index
            tapHandler:(void (^)(YSTabItem *item))handler;

/**
 *  当YSTabBar所属的YSTabBarController内容视图支持拖动切换时，
 *  此方法用于同步内容视图scrollView拖动的偏移量，以此来改变YSTabBar内控件的状态
 */
