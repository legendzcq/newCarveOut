...
@implementation NSString (MRTool)
...
//    NSString *phoneRegex = @"^((13[0-9])|(15[^4,\\D])|(18[0,0-9])|(17[0,0-9])|(147)|(170))\\d{8}$";
...
+ (BOOL) validateUserName:(NSString *)name{
    NSString *userNameRegex = @"^[\u4E00-\u9FA5A-Za-z0-9_]{2,15}$";
...
+ (NSString *)replaceStringWithMobileString:(NSString*)string{
    if (string.length>=11) {
        NSRange range = NSMakeRange(3, 1);
...
//自适应小数点
+ (NSString*)validateFloatValue:(NSNumber*)value{
    ...
    }else {return [NSString stringWithFormat:@"%.2f",f];
