...
@implementation MRProgressLayer
...
- (void)progressChanged:(NSTimer *)timer {
    ...
    if (_progressStyle == DKProgressStyle_Gradual) {
        ...
        _gradientLayer.frame = CGRectMake(0, 0, kWidth*self.strokeEnd, 2);
    }
}
...
- (void)progressAnimationCompletion {
    ...
    self.stepWidth = 0.01;
    self.strokeEnd = 0;
    self.gradientLayer.locations = @[@(self.strokeEnd/2), @(self.strokeEnd)];
    self.gradientLayer.frame = CGRectMake(0, 0, 0, 2);
}
